...
export class ProfitableMarketLogRepository {
  ...
  async findProfitableLogsByAnalysisType(
    analysisType: string,
  ): Promise<ProfitableMarketLog[]> {
    return await this.profitableMarketLogModel
      .find({ analysisType })
  ...
  async getProfitableLogsStats(): Promise<{
    ...
    const topLog = await this.profitableMarketLogModel
      .findOne()
      .sort({ maxPriceChangePercent: -1 })
      .exec();
