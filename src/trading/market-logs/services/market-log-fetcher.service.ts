...
export class MarketLogFetcherService {
    ...
    for (const from of ASSETS) {
      ...
      for (const interval of timeframes) {
        ...
        if (interval === '5m') {
          ...
          if (candles?.[0]) {
            logDto.change_3min = this.calcUtils.calcPercentDiffBetweenPrices(
              candlesData[0].close,
              currentPrice,
            );
          }
          if (candles?.[1]) {
            logDto.change_6min = this.calcUtils.calcPercentDiffBetweenPrices(
              candlesData[1].close,
              currentPrice,
            );
          }
          if (candles?.[2]) {
            logDto.change_9min = this.calcUtils.calcPercentDiffBetweenPrices(
              candlesData[2].close,
              currentPrice,
            );
          }
        ...
        if (interval === '15m') {
          ...
          if (candles?.[0]) {
            logDto.change_15min = this.calcUtils.calcPercentDiffBetweenPrices(
              candlesData[0].close,
              currentPrice,
            );
          }
          if (candles?.[1]) {
            logDto.change_30min = this.calcUtils.calcPercentDiffBetweenPrices(
              candlesData[1].close,
              currentPrice,
            );
          }
          if (candles?.[2]) {
            logDto.change_45min = this.calcUtils.calcPercentDiffBetweenPrices(
              candlesData[2].close,
              currentPrice,
            );
          }
        ...
        if (interval === '1h') {
          ...
          if (candles?.[0]) {
            logDto.change_1h = this.calcUtils.calcPercentDiffBetweenPrices(
              candlesData[0].close,
              currentPrice,
            );
          }
          if (candles?.[1]) {
            logDto.change_2h = this.calcUtils.calcPercentDiffBetweenPrices(
              candlesData[1].close,
              currentPrice,
            );
          }
          if (candles?.[2]) {
            logDto.change_3h = this.calcUtils.calcPercentDiffBetweenPrices(
              candlesData[2].close,
              currentPrice,
            );
          }
        ...
        if (interval === '4h') {
          ...
          if (candles?.[0]) {
            logDto.change_4h = this.calcUtils.calcPercentDiffBetweenPrices(
              candlesData[0].close,
              currentPrice,
            );
          }
          if (candles?.[1]) {
            logDto.change_8h = this.calcUtils.calcPercentDiffBetweenPrices(
              candlesData[1].close,
              currentPrice,
            );
          }
          if (candles?.[2]) {
            logDto.change_12h = this.calcUtils.calcPercentDiffBetweenPrices(
              candlesData[2].close,
              currentPrice,
            );
          }
  ...
  async findUncheckedProfitabilityLogsBatch(
    ...
    return await this.repository.findUncheckedProfitabilityLogsBatch(
      batchSize,
      skip,
    );
  }
  ...
  async findUncheckedProfitabilityLogsBatchFromLast24Hours(
    ...
    return await this.repository.findUncheckedProfitabilityLogsBatchFromLast24Hours(
      batchSize,
      skip,
      last24Hours,
    );
  ...
  async findUncheckedProfitabilityLogsBatchFromLastDays(
    batchSize: number,
    skip: number,
    startDate: Date,
  ): Promise<MarketLog[]> {
    return await this.repository.findUncheckedProfitabilityLogsBatchFromLastDays(
      batchSize,
      skip,
      startDate,
    );
  ...
  async findLogsByAssetAndTimeWindow(
    ...
    return await this.repository.findLogsByAssetAndTimeWindow(
      from,
      startTime,
      hoursWindow,
    );
  ...
  async findProfitableLogsFromLastDays(startDate: Date): Promise<MarketLog[]> {
    return await this.repository.findProfitableLogsFromLastDays(startDate);
  }
}
