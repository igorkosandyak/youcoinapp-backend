...
@Injectable()
export class BinanceFetcherService implements ExchangeFetcherService {
  private readonly logger = new Logger(BinanceFetcherService.name);
  ...
  async getSpotLatestPrices(): Promise<Record<string, number>> {
    try {
      const response = await this.BINANCE.fetchTickers();
  ...
  async getSpotOrderbook(from: string, to: string): Promise<any> {
    const response = await this.BINANCE.fetchOrderBook(from + to);
    return response;
  }
}
